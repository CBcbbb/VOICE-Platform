...
# FIXED IMPORTS - Use the same as your working populate_data.py
...
def initialize_data(db: Session = Depends(get_db)):
    """Initialise database with sample data via API"""
    try:
        # Clear existing data
        db.query(Link).delete()
        db.query(Node).delete()

        # Add sample nodes using the same approach as populate_data.py
        sample_nodes = [
            Node(
                id="P001",
                name="Jakob Kukula",
            ...
            Node(
                ...
                name="Marina Wainer",
                type="People",
                bio="Paris-based artist making interactive art",
            ...
            Node(
                ...
                name="RiverSync",
                type="Projects",
                description="Transdisciplinary artistic intervention focused on river systems",
                budget="12,000 Euros"
            ),
            Node(
                id="M001",
                ...
                type="Methods",
                description="Immersive, participatory approach for exploring human-nature relationships",
        ...
        # Add sample links
        sample_links = [
            Link(id="L001", source_id="P001", target_id="PR001", relationship_type="leads", strength=1.0),
            Link(id="L002", source_id="P001", target_id="I001", relationship_type="mentored_by", strength=0.7),
            Link(id="L003", source_id="P002", target_id="I001", relationship_type="mentored_by", strength=0.7),
            Link(id="L004", source_id="PR001", target_id="M001", relationship_type="applies", strength=0.9)
        ]
        ...
        db.commit()
        return {"message": "Database initialized successfully!", "nodes": len(sample_nodes), "links": len(sample_links)}
